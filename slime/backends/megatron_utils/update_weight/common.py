    ...
    for vp_stage, model_module in enumerate(model):
        ...
        for name, param in model_module.named_parameters():
            ...
            if match:
                ...
                if os.environ.get("QWEN35_DEBUG_GATHER") and layer_idx == layer_offset:
                    print(
                        f"[QWEN35_DEBUG_GATHER] ep_rank={ep_rank} offset={expert_offset} "
                        f"name={rest} shape={tuple(param.shape)} "
                        f"tp={getattr(param, 'tensor_model_parallel', None)} "
                        f"pdim={getattr(param, 'partition_dim', None)} "
                        f"pstride={getattr(param, 'partition_stride', None)}",
                        flush=True,
                    )
